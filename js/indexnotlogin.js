        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                for (var i = 0; i < arr.length; i++) {
                    title += `
                <li data-id="${arr[i].pid}" class="notopoenclass border-bottom row ml-0 mb-3">
                <a class="${!arr[i].pic ? "col-3" : ''}" href="detail.html?pid=${arr[i].pid}">
                    <${!!arr[i].pic ? 'img' : 'any'} src="${urlhttp}${arr[i].pic}" class="${arr[i].pic != 'null' ? 'indeximgsss' : ''}">
                </a>
                <div class="col-9">
                <div class="row pl-0 mt-lg-2 notopoenclass2">
                <a class="col-12" href="detail.html?pid=${arr[i].pid}"><span class="font-size-1">${arr[i].title}</span></a>
                <div class="col-12 mt-3 pt-2 pl-3 mb-3 align-items-center">
                <a class="mr-3" href="detail.html?pid=${arr[i].pid}#source">
